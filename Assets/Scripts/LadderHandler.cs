...
public class LadderHandler : MonoBehaviour {
    ...
    void OnValidate() {
        _boxCollider2D = GetComponent<BoxCollider2D>();
        _boxCollider2D.size = new Vector2(_boxCollider2D.size.x, Mathf.Clamp(LadderSize * 2, 4, LadderSize*2 + 2));
        transform.GetChild(0).localPosition = new Vector3(0, (LadderSize - 1));
        transform.GetChild(1).GetComponent<SpriteRenderer>().size =
            new Vector2(transform.GetChild(1).GetComponent<SpriteRenderer>().size.x, 2 * (LadderSize - 2));
        transform.GetChild(2).localPosition = new Vector3(0, -(LadderSize - 1));
    }
}
